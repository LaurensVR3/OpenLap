...
# Gauge geometry
_MAX_SPEED  = 300.0   # km/h at full scale
_ANG_START  = 220.0   # degrees CCW from +x axis (= bottom-left, = 0 km/h)
_ANG_SWEEP  = 240.0   # total sweep (CW → decreasing angle)


def _spd_to_rad(speed: float) -> float:
    frac = min(1.0, max(0.0, speed / _MAX_SPEED))
    return math.radians(_ANG_START - frac * _ANG_SWEEP)


def _arc_xy(r: float, v_start: float, v_end: float, n: int = 80):
    a1 = math.radians(_ANG_START - (v_start / _MAX_SPEED) * _ANG_SWEEP)
    a2 = math.radians(_ANG_START - (v_end   / _MAX_SPEED) * _ANG_SWEEP)
...
def render(data: dict, w: int, h: int):
    ...
    def draw_arc(v0, v1, col, lw, r=R):
        xs, ys = _arc_xy(r, v0, v1)
        ax.plot(xs, ys, color=col, lw=lw,
                solid_capstyle='butt', zorder=2)

    # Track (grey background)
    draw_arc(0, _MAX_SPEED, '#1a2a3a', LW_TRACK + 2)
    # Colour bands
    draw_arc(0,   150, '#1a6633', LW_TRACK)
    draw_arc(150, 220, '#7a7a00', LW_TRACK)
    draw_arc(220, _MAX_SPEED, '#882200', LW_TRACK)

    # Live speed fill
    spd_col = '#00ff88' if spd < 150 else ('#ffcc00' if spd < 220 else '#ff4444')
    if spd > 0:
        draw_arc(0, min(spd, _MAX_SPEED), spd_col, LW_LIVE)

    # ── Tick marks and labels ─────────────────────────────────────────────────
    fs_tick = max(5, int(8 * sc))
    for v in range(0, int(_MAX_SPEED) + 1, 50):
        ang  = _spd_to_rad(v)
    ...
    # ── Needle ────────────────────────────────────────────────────────────────
    needle_ang = _spd_to_rad(min(spd, _MAX_SPEED))
    needle_len = 0.78
