...
def render(data: dict, w: int, h: int):
    ...
    ax.text(0.50, 0.31, f"{m:02d}:{s:05.2f}",
    ...
    ax.add_patch(plt.Rectangle((0.08, 0.10), 0.84, 0.07,
                 facecolor='#1a2a3a', zorder=2))
    ax.add_patch(plt.Rectangle((0.08, 0.10), 0.84 * prog, 0.07,
                 facecolor=bar_col, zorder=3))

    # G readout (tiny, below progress bar)
    if is_bike:
        ...
        side_col = '#7799bb'
    ax.text(0.50, 0.035, side_txt,
            ha='center', va='bottom', color=side_col,
