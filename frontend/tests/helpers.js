...
export function makeAPI(overrides = {}) {
  return {
    ...
    getVideoLayers:     vi.fn(async () => []),
    setSecondCamera:    vi.fn(async () => ({})),
    setSecondCameraOffset: vi.fn(async () => null),
    renamePreset:       vi.fn(async () => null),
    deletePreset:       vi.fn(async () => null),
    saveOverlay:        vi.fn(async () => null),
    listPresets:        vi.fn(async () => []),
    getTrackLines:      vi.fn(async () => ({ outline: { lats: [], lons: [] }, finish: null, sectors: [], user: false })),
    setTrackLine:       vi.fn(async () => ({ outline: { lats: [], lons: [] }, finish: null, sectors: [], user: false })),
    getReferencePreview: vi.fn(async () => ({ ok: false, desc: 'none' })),
    getWeather:         vi.fn(async () => ({ weather: '—', wind: '—' })),
